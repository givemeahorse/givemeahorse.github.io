...
写这个游戏使用了AI产品来辅助，例如chatGPT帮助代码难点的实现、SD，midjourney帮助UI图标和app图标的生成。这让我对于未来游戏开发有了一些思考，例如：原画、定型、建模、材质、骨骼绑定、动画的过程是个体力与耐心堆砌的过程，这也是我有一些角色灵感素描出来的时候却不敢真正进一步实施的原因，那么从原画到生成模型和模型到刷权重、骨骼绑定、K帧动画是否可以借助AI进行完成，这至少对于个人开发者来说是个乐观的畅想。例如Wonder Studio、Genie、Tripo的智能生产3d模型的过程，以及类似于sora、Pika、Stable Video Diffusion的图片生成视频对于游戏CG、转场、宣发的促进作用，甚至对于下沉市场的短平快的低端游戏的一体成型。自然语言到解决方案、图片、视频、模型、地编、剧情策划、战斗设计，
